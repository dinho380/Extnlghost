...
function setOption(key, value, force)
  ...
  elseif key == 'enableLights' then
    gameMapPanel:setDrawLights(value and options['ambientLight'] < 2)
    graphicsPanel:getChildById('ambientLight'):setEnabled(value)
    graphicsPanel:getChildById('ambientLightLabel'):setEnabled(value)
  elseif key == 'ambientLight' then
    graphicsPanel:getChildById('ambientLightLabel'):setText(tr('Ativar Luz Maxima: %s%%', value))
    gameMapPanel:setMinimumAmbientLight(value/1)
    gameMapPanel:setDrawLights(options['enableLights'] and value < 2)
